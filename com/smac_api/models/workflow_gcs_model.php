...
class workflow_gcs_model extends kol_model{
	/*
	private $request;
	...
	function feeds($campaign_id,$folder_id,$keyword,$start,$limit=10){
		...
		if($folder_id!=4){
			...
			$sql = "SELECT a.id as feed_id, author_name,link,author_uri, comments,title,summary as content,published_ts, source_service,
					...
					AND c.folder_id={$folder_id} AND c.keyword='{$keyword}' 
	...
	function website_detail($campaign_id,$sitename){
		$summary = array('sitename'=>$sitename,"wordcloud"=>$this->web_wordcloud($campaign_id,$sitename));
		
		return $summary;
	}
	function web_wordcloud($campaign_id,$sitename){
	...
	function exclude_all($campaign_id,$keyword){
		$sql = "INSERT IGNORE INTO smac_workflow.gcs_workflow_apply_exclude
			...
			if($cek['campaign_id']==null){
				...
				$sql = "SELECT feed_id,
						...
						AND NOT EXISTS (SELECT 1 FROM smac_workflow.workflow_marked_gcs mt 
										...
										AND mt.campaign_web_feeds_id=cf.id LIMIT 1)
						LIMIT {$start},{$total};";
				...
				$sql = "SELECT COUNT(*) AS total FROM (
								...
								WHERE keyword='{$keyword}' GROUP BY campaign_web_feeds_id) a;";
	...
	function flag_feeds($campaign_id,$keyword,$feed_id,$folder_id){
		...
		$sql = "SELECT id,group_type_id FROM smac_report.campaign_web_feeds 
				WHERE campaign_id={$campaign_id} AND feed_id = {$feed_id} LIMIT 1";
		$rs = $this->fetch($sql);
		return $this->flag($campaign_id,$keyword,$rs['id'],$folder_id,$rs['group_type_id']);
	}
	function flag($campaign_id,$keyword,$feed_id,$folder_id,$group_type_id){
		if(strlen($keyword)==0){
			$keyword = "N/A";
		}
		$sql="INSERT IGNORE INTO smac_workflow.workflow_marked_gcs
			 ...
			 VALUES(".$campaign_id.",'".$feed_id."','".$keyword."',".$folder_id.",".time().",NOW(),{$group_type_id})";	
		$q = mysql_query($sql);
		if($q){
			return 1;
		}else{
		...
		if($type==1){
			$sql = "SELECT a.id AS feed_id,a.feed_id as fid,author_name,link,author_uri, comments,title,summary AS content,published_ts, source_service,
					...
					AND NOT EXISTS (SELECT 1 FROM smac_workflow.workflow_marked_gcs c
									WHERE c.campaign_web_feeds_id = a.id LIMIT 1)
					LIMIT {$start},{$total}
				";
