...
if($lang=="all"){
	if($exclude==1){
		$sql = "SELECT author_id as author,author_avatar as pic,sentiment as total,pii_score 
			FROM smac_market.campaign_ambas
		...
		$sql = "SELECT author_id as author,author_avatar as pic,sentiment as total,pii_score 
			FROM smac_market.campaign_ambas
			WHERE campaign_id=".$campaign_id." AND geo='{$geo}' ORDER BY sentiment DESC LIMIT 6";
	}
}else{
	$sql = "SELECT a.author_id as author,a.author_avatar as pic,sentiment as total,pii_score 
			FROM smac_market.campaign_ambas a
			 INNER JOIN smac_market.campaign_people_lang b
			ON a.author_id = b.author_id 
			WHERE a.campaign_id=".$campaign_id." AND a.geo='{$geo}' AND b.campaign_id=".$campaign_id." AND b.geo='{$geo}' 
	 		ORDER BY sentiment DESC LIMIT 6";
}
...
if($lang=="all"){
	if($exclude==1){
		$sql = "SELECT author_id as author,author_avatar as pic,sentiment as total,pii_score 
			FROM smac_market.campaign_trolls
			WHERE campaign_id=".$campaign_id." AND a.geo='{$geo}' 
			AND author_id NOT IN (SELECT author_id FROM smac_report.kol_group WHERE account_type=1)
	...
	$sql = "SELECT a.author_id as author,a.author_avatar as pic,sentiment as total,pii_score 
		FROM smac_report.campaign_trolls a
		INNER JOIN smac_report.campaign_people_lang b
		ON a.author_id=b.author_id		
