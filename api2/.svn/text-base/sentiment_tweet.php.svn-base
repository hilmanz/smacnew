...
if($type==1){
	$sql = "SELECT * FROM (
					...
					WHERE b.campaign_id=".$campaign_id." AND c.country_code = '{$geo}' 
	...
	$sql2 = 
		"SELECT COUNT(*) as total 
		 FROM (SELECT published_date,author_id,author_name,author_avatar,content,sentiment,followers as imp 
		 	FROM smac_report.campaign_feed_sentiment a
			INNER JOIN {$FEEDS} b
 				ON a.feed_id = b.id AND a.campaign_id = ".$campaign_id."			 
			INNER JOIN smac_report.country_twitter c
				ON b.feed_id = c.feed_id
		 WHERE b.campaign_id=".$campaign_id." AND c.country_code = '{$geo}' AND a.sentiment > 0 AND b.published_date = '".$day."') c LIMIT 1";
	...
	$sql = 
		"SELECT * FROM (
			...
			FROM smac_report.campaign_feed_sentiment a
				...
				INNER JOIN smac_report.country_twitter c
					ON b.feed_id = c.feed_id
			WHERE b.campaign_id=".$campaign_id." AND c.country_code = '{$geo}' AND (a.sentiment > 0 OR a.sentiment < 0)) c
			ORDER BY c.imp DESC LIMIT ".$start.",".$perpage;
	...
	$sql2 = 
		"SELECT COUNT(*) as total FROM (
			...
			FROM smac_report.campaign_feed_sentiment a
				...
				INNER JOIN smac_report.country_twitter c
					ON b.feed_id = c.feed_id
			WHERE b.campaign_id=".$campaign_id." AND c.country_code = '{$geo}' AND (a.sentiment > 0 OR a.sentiment < 0)) c LIMIT 1";
	...
	$sql = 
		"SELECT * FROM (
			...
			FROM smac_report.campaign_feed_sentiment a
				...
				INNER JOIN smac_report.country_twitter c
					ON b.feed_id = c.feed_id
			WHERE b.campaign_id=".$campaign_id." AND c.country_code = '{$geo}' AND a.sentiment < 0 AND b.published_date = '".$day."') c
			ORDER BY c.imp DESC LIMIT ".$start.",".$perpage;
	...
	$sql2 = 
		"SELECT COUNT(*) as total FROM (
			...
			FROM smac_report.campaign_feed_sentiment a
				...
				INNER JOIN smac_report.country_twitter c
					ON b.feed_id = c.feed_id
			WHERE b.campaign_id=".$campaign_id." AND c.contry_code='{$geo}' AND a.sentiment < 0 AND b.published_date = '".$day."') c LIMIT 1";
}
