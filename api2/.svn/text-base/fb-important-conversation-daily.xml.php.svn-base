...
if($lang=='all'){
	$sql = "SELECT a.feed_id,from_object_id,from_object_name,likes_count,message,created_time,b.created_time_ts,application_object_name FROM smac_report.campaign_fb_history a
	INNER JOIN smac_data.feeds_facebook b
	ON a.feed_id = b.id
	WHERE a.campaign_id=".$campaign_id." AND b.message <> '' AND b.created_time_ts>=".$from_date." AND b.created_time_ts<=".$to_date."
...
if($num > 0){
	while($r=mysql_fetch_array($q)){
		...
		$xml.="<name>".$r['from_object_name']."</name>\n";
		$xml.="<url>http://www.facebook.com/".$r['from_object_id']."</url>";
		$xml.="<txt>".htmlspecialchars($r['message'])."</txt>\n";
